function [Yp, Z, trend] = forecast_SRPGP(model, X, regr) 
% BLUP for double check
%         r = dmodel.corr(delta, theta, p, 1, (1:n)'); 
%         r(1) = r(1) + delta^2;    S = toep_chol(r);
%         gamma = S' \ (S \ (dmodel.Y-dmodel.H*dmodel.betahat));
%         Cr = dmodel.corr(delta, theta, p, X, (1:n)');
%         Yhat = f*dmodel.betahat + Cr*gamma;
    n = length(model.Y);    q = size(model.H,2);
  	T = model.period;    p = str2num(regexprep(rats(T, 30),'/.*', '')); % decimal period
    k = floor(n/p);    p1 = n - p*k;
    theta = model.thetahat(2);   alpha = model.alpha;
    model.res = model.Y - model.H*alpha - ifft(model.beta)*sqrt(n);
    Y = model.res(1:p*k);    Yb = mean(reshape(Y, p, k), 2);    Ys = model.res(1+p*k:end);

    delta = model.thetahat(1);    H = regr(X/n);
    if p > n
        ...
        RRdYb = ifft(RRd.eig .* fft(Yb));    Ybl = ifft(fft(Yb) ./ Rd.eig);
        if p1>0
        ...
        [X, ~, index] = unique(mod(X,p));
        Cr = model.corr(delta, theta, p, X, (1:p)');
        Zb = (k/delta^2)*(Cr*Ybl);
        if p1>0
            Cs = model.corr(delta, theta, p, X, (1:p1)');
            res = zeros(p,1);    res(1:p1) = Ydll;
            Zd = Cs*Ydll - Cr* ifft(RRd.eig .* fft(res));
            Zb = Zb + Zd;
        end
        Z = Zb(index);
        trend = H*alpha;    Yp = trend + Z;
    end
end
