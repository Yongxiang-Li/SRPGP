function [Yp, Z, trend] = forecast_CPGP(model, X, regr) 
...
%         Yhat = f*dmodel.betahat + Cr*gamma;
    ...
    delta = model.thetahat(1);    F = regr(X/n);
    if p > n
        ...
        Gbl = ifft(fft(Gb) ./ Rd.eig);    Ybl = ifft(fft(Yb) ./ Rd.eig);
        if p1>0
        ...
        if nargin>1
            ...
            Zb = (k/delta^2)*(Cr*(Ybl-Gbl*beta));
            if p1>0
                Cs = model.corr(delta, theta, p, X, (1:p1)');
                res = zeros(p,1);    res(1:p1) = Ydll-Gdll*beta;
                Zd = Cs*(Ydll-Gdll*beta) - Cr* ifft(RRd.eig .* fft(res));
                Zb = Zb + Zd;
            end
            Z = Zb(index);
        else
